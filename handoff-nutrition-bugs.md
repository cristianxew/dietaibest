...
> **STATUS 2026-06-26: ALL THREE BUGS FIXED.** #1 (prior session), #2 + #3 (this
> session). Branch: `feature/nutrition-calc-improvements`. tsc clean · 794 unit
> pass (89 files) · eval gate 29 pass / 2 skip. Details inline below each bug.
...
## ✅ FIXED bug #2 — chat `getNutrition` drops the recipe title (clear fix)
**Fix:** `analyzeFresh` ([src/lib/chat/tools/getNutrition.ts](src/lib/chat/tools/getNutrition.ts)) gained an optional `title?` param, forwarded as `analyzeRecipeProfileAction({ ingredients, servings, title })`. The recipe path passes `r.title` (getRecipe spreads the full Prisma row, so it's present); the ad-hoc ingredient-list path legitimately has no title. Test added: "forwards the recipe title to fresh analysis" in [tests/unit/chat/getNutrition.test.ts](tests/unit/chat/getNutrition.test.ts).
...
## ✅ FIXED bug #3 — Stage-2 `chosenFdcId: null` discards the deterministic backstop (TRADEOFF — resolved by decision + ADR addendum)
**Decision (grilled with owner) + fix:** a null selection splits on `flagged`. A *confident* null (`flagged: false`) is the LLM's sovereign "no candidate is reasonable" → estimate (unchanged). A *failed / low-confidence* null (`flagged: true`) recovers the deterministic pick **only when it is the curated staple pin** (`stapleFdcId(name) === batch.food.fdcId`) — never a plain search match, so the cooked/raw trap can't reopen (the staple map excludes dry grains, pins legumes cooked-consistent). Recovered picks trace `selectedVia: "staple-backstop"`. Recorded as the [ADR 0004 addendum](docs/adr/0004-llm-assisted-food-resolution.md) + CONTEXT.md "Staple backstop" term + nutrition_units.md Stage-2 section. Tests: 3 in [tests/unit/resolve-ingredients.test.ts](tests/unit/resolve-ingredients.test.ts) (recover / sovereign-confident-null / staple-pin-only boundary).
