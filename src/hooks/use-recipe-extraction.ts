...
export function useRecipeExtraction(
  ...
  const extract = useCallback(async (url: string): Promise<ImportedRecipe> => {
    if (isMountedRef.current) {
      setState((prev) => ({
        ...prev,
        status: "starting",
      ...
      return new Promise<ImportedRecipe>((resolve, reject) => {
        ...
        if (useSSE) {
          ...
          eventSource.onerror = () => {
            ...
            const translatedMsg = getErrorMessage("Lost connection to extraction service");
            if (isMountedRef.current) {
              setState((prev) => ({
                ...prev,
                status: "error",
          ...
          const executePoll = async () => {
            while (pollCount < maxPolls) {
              ...
              if (!isMountedRef.current || rejectPromiseRef.current !== reject) {
                return;
              }
              ...
              try {
                ...
                if (
                  ...
                  throw new Error(statusResult.message || "Extraction failed");
                }
              } catch (error) {
            ...
            // Exceeded max polls
            const translatedMsg = getErrorMessage("Recipe extraction took too long");
            if (isMountedRef.current) {
              setState((prev) => ({
                ...prev,
                status: "error",
