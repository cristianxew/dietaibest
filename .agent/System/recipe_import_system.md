...
### useRecipeExtraction.ts - State Machine Owner

**Location:** `/src/hooks/use-recipe-extraction.ts`

The extraction logic has been centralized into the `useRecipeExtraction` hook. Components like `EntryScreen.tsx` are now thin UI wrappers that consume this hook. The legacy components `URLUpload.tsx`, `RecipeImport.tsx`, `ImageUpload.tsx`, and `PDFUpload.tsx` have been removed to enforce this centralized architecture.

**Hook Responsibilities:**
- Managing the full extraction state machine (idle -> validation -> polling/sse -> success/error).
- Managing `EventSource` lifecycle and cleanup.
- Handling `AbortController` for network requests.
- Providing a `reset()` method to clear the state and allow for retries or new imports.
- Translating technical errors into user-friendly UI messages.
